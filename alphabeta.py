            ...
            if(type == 'min'):
                for i in range(0,len(node.array)):
                    if (node.array[i] == 0):
                        ...
                        son.father = node
                        if (node.father != None):
                            if (node.beta != None and node.father.alpha != None):
                                if (node.beta <= node.father.alpha):
                                    break
            ...
            if (type == 'max'):
                for i in range(0,len(node.array)):
                    if (node.array[i] == 0):
                        ...
                        son.father = node
                        if(node.father!=None):
                            if(node.alpha!=None and node.father.beta!=None):
                                if(node.alpha>=node.father.beta):
                                    break
                        son = self.buildTree(son,'min')
                        if(node.value == None or son.value>node.value):
                            node.value = son.value
                            node.alpha = son.value
                            node.choice = son
                return node